# -*- coding: utf-8 -*-
"""
Build exploratory-experiments team report (Markdown + DOCX + PDF).
Run from repo root:
...
REPORT_MD = """# Exploratory Experiments — Team Report
...
def write_docx(text: str, path: Path):
    ...
    for block in text.split("\n"):
        ...
        elif block.startswith("|") and "---" not in block:
            # keep as paragraph; full table formatting omitted for brevity
...
def write_pdf(text: str, path: Path):
    ...
    for block in text.split("\n"):
        if block.startswith("# "):
            ...
            story.append(Paragraph(escape(block), styles["Normal"]))
            story.append(Spacer(1, 0.08 * inch))
    doc.build(story)


def main():
    md_path = DOCS / "Exploratory_Experiments_Team_Report.md"
    md_path.write_text(REPORT_MD, encoding="utf-8")
    try:
        write_docx(REPORT_MD, DOCS / "Exploratory_Experiments_Team_Report.docx")
        print("Wrote DOCX")
    except Exception as e:
        print("DOCX failed:", e)
    try:
        write_pdf(REPORT_MD, DOCS / "Exploratory_Experiments_Team_Report.pdf")
        print("Wrote PDF")
    except Exception as e:
        print("PDF failed:", e)
