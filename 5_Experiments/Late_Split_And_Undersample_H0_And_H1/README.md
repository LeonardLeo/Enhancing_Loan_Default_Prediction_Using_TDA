# Late split and undersample (the original historical run), using both H0 and H1

Folder: `Late_Split_And_Undersample_H0_And_H1`

Public name (figures and paper tables): **Late split and undersample (the original historical run), using both H0 and H1**. Names come from `utils.process_display_name()`.
...
4. `8_Permutation_Test_Of_Class_Difference` — permutation test on this process's barcode vectors
5. `9_Revised_Snapshot_Protocol` — fixed points per snapshot; default 60 training / 15 test snapshots

The matching just-H0 process is `Late_Split_And_Undersample_H0/`. Nested extras (H0-only copies, drop-correlated, linear regression, mean/variance) live under `Archives/Four_Arm_Nested_Experiments/Historical_Late_Split_Balanced_TDA/`.
