...
Run from repo root:
  .\\tda_env\\Scripts\\python.exe 5_Experiments/28_Revised_Snapshot_Protocol/run_protocol.py

Stages:
  1) design   — ID estimation, formula vs reuse (separated), overlap planning
  2) split_ml — early-split PH + models (default 60/15 + Zaniar sweeps)
  3) full_ml  — DCCCD non-split l in {60,75,90}
  4) all      — everything

Resume-safe: skips completed run keys found in results CSVs / marker JSONs.
"""
...
RESULTS = REPO_ROOT / "6_Results" / "28_Revised_Snapshot_Protocol"
DATA_LANDMARKS = REPO_ROOT / "1_Data" / "Landmark_Sets"
DATA_TDA = REPO_ROOT / "1_Data" / "TDA_Datasets"
EXP_NAME = "28_Revised_Snapshot_Protocol"
...
DATASET_SPECS = {
    "credit_card_default": {
        ...
        "pca_components": 7,
        "run_full_nonsplit": True,  # bigger dataset — Zaniar 60–90
    },
    "statlog_german": {
        "path": REPO_ROOT
...
def design_for_dataset(dataset_key: str) -> dict:
    ...
    # PCA space used by TDA (train-only impute/scale/PCA)
    Xtr, Xte, ytr, yte, var = early_split_pca(
        X, y, n_components=spec["pca_components"], random_state=42
    )
    ...
    train_pos = int((ytr == cfg.positive_label).sum())
    train_neg = int((ytr != cfg.positive_label).sum())
    test_pos = int((yte == cfg.positive_label).sum())
    test_neg = int((yte != cfg.positive_label).sum())
    ...
    design = {
        ...
        "pca_variance_retained_train_fit": var,
...
def run_split_setting(
    ...
    lm_train = (
        DATA_LANDMARKS / folder / EXP_NAME / f"split_t{t}_tr{train_l}_te{test_l}" / "train"
    )
    lm_test = (
        DATA_LANDMARKS / folder / EXP_NAME / f"split_t{t}_tr{train_l}_te{test_l}" / "test"
    )
    tda_dir = DATA_TDA / folder / EXP_NAME / f"split_t{t}_tr{train_l}_te{test_l}"
    tda_dir.mkdir(parents=True, exist_ok=True)

    meta_tr = generate_fixed_t_snapshots(
        train_classes, t=t, l=train_l, output_root=lm_train, tag="train", random_state=42
    )
    meta_te = generate_fixed_t_snapshots(
        test_classes, t=t, l=test_l, output_root=lm_test, tag="test", random_state=43
    )
        ...
        row.update(
            {
                "run_key": run_key,
                "dataset": dataset_key,
                "protocol": "early_split_no_undersample_fixed_t",
                "mode": mode,
                "t": t,
                "train_l": train_l,
                "test_l": test_l,
...
def run_full_nonsplit(dataset_key: str, design: dict, t: int, l: int) -> None:
    ...
    n_comp = min(spec["pca_components"], Xs.shape[0] - 1, Xs.shape[1])
    ...
    lm = DATA_LANDMARKS / folder / EXP_NAME / f"full_t{t}_l{l}"
    meta = generate_fixed_t_snapshots(classes, t=t, l=l, output_root=lm, tag="full", random_state=42)
    ...
    tda_dir = DATA_TDA / folder / EXP_NAME / f"full_t{t}_l{l}"
    tda_dir.mkdir(parents=True, exist_ok=True)
